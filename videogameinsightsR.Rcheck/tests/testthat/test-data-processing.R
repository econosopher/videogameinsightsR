...
test_that("Rankings data is processed correctly", {
  httptest2::with_mock_api({
    ...
    if (!is.null(result)) {
      ...
      expected_cols <- c("steam_app_id", "total_revenue_rank", "total_revenue_prct")
      expect_true(all(expected_cols %in% names(result)))
      expect_type(result$steam_app_id, "integer")
      expect_type(result$total_revenue_prct, "double")
      if (nrow(result) > 1) {
        non_na_ranks <- result$total_revenue_rank[!is.na(result$total_revenue_rank)]
...
test_that("Metadata batch processing handles large inputs", {
  httptest2::with_mock_api({
    ...
    if (!is.null(result)) {
      expect_s3_class(result, "data.frame")
      if (nrow(result) > 0) {
        expect_true("steam_app_id" %in% names(result))
        expect_true("name" %in% names(result))
      }
    }
  })
