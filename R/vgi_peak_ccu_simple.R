...
vgi_peak_ccu_by_ids <- function(steam_app_ids) {
  if (is.null(steam_app_ids) || length(steam_app_ids) == 0) {
    return(tibble::tibble(name = character(), steamAppId = integer(), peak_ccu = numeric(), peak_date = as.Date(character())))
  }
...
vgi_peak_ccu_by_names <- function(game_names) {
  if (is.null(game_names) || length(game_names) == 0) {
    return(tibble::tibble(input_name = character(), name = character(), steamAppId = integer(), peak_ccu = numeric(), peak_date = as.Date(character())))
  }
  ...
  peak_df[, c("input_name", "name", "steamAppId", "peak_ccu", "peak_date")]
}
